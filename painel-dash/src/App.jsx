    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 xl:gap-6">
          <CardMetaNova titulo="Faturamento Geral" valor={formatarAbrev(dadosMetas?.realizado_total_geral)} percentual={calcPerc(dadosMetas?.realizado_total_geral, dadosMetas?.meta_total_geral)} labelMeta="Meta Faturamento:" valorMeta={formatarAbrev(dadosMetas?.meta_total_geral)} onClickExpandir={() => abrirModalValExp('Faturamento Geral', formatarMoeda(dadosMetas?.realizado_total_geral), 'Soma da receita válida de todas as estruturas.')} />
          <CardMetaNova titulo="Realizado Diário" valor={formatarAbrev(dados?.realizado_diario)} percentual={calcPerc(dados?.realizado_diario, dados?.meta_diaria)} labelMeta="Meta Diária:" valorMeta={formatarAbrev(dados?.meta_diaria)} onClickExpandir={() => abrirModalValExp('Realizado Diário', formatarMoeda(dados?.realizado_diario), 'Receita total do dia atual.')} />
          <CardMetaNova titulo="Atividade Geral" valor={`${Number(dadosMetas?.percentual_atividade_total_geral||0).toFixed(1)}%`} percentual={calcPerc(dadosMetas?.percentual_atividade_total_geral, dadosMetas?.meta_atividade_geral)} labelMeta="Meta Atividade:" valorMeta={`${Number(dadosMetas?.meta_atividade_geral||0).toFixed(1)}%`} onClickExpandir={() => abrirModalValExp('Atividade Geral', Number(dadosMetas?.atividade_total_geral || 0).toLocaleString('pt-BR'), 'Total de revendedores ativados.')} />
          <CardMetaNova titulo="MAKE Geral" valor={`${Number(dadosMetas?.percentual_make_total_geral||0).toFixed(1)}%`} percentual={calcPerc(dadosMetas?.percentual_make_total_geral, dadosMetas?.meta_make_geral)} labelMeta="Meta MAKE:" valorMeta={`${Number(dadosMetas?.meta_make_geral||0).toFixed(1)}%`} onClickExpandir={() => abrirModalValExp('MAKE Geral', Number(dadosMetas?.make_total_geral || 0).toLocaleString('pt-BR'), 'Total de ativados com a categoria MAKE.')} />
          <CardMetaNova titulo="CABELO Geral" valor={`${Number(dadosMetas?.percentual_cabelo_total_geral||0).toFixed(1)}%`} percentual={calcPerc(dadosMetas?.percentual_cabelo_total_geral, dadosMetas?.meta_cabelo_geral)} labelMeta="Meta CABELO:" valorMeta={`${Number(dadosMetas?.meta_cabelo_geral||0).toFixed(1)}%`} onClickExpandir={() => abrirModalValExp('CABELO Geral', Number(dadosMetas?.cabelo_total_geral || 0).toLocaleString('pt-BR'), 'Total de ativados com a categoria CABELO.')} />
          <CardMetaNova titulo="RPA Geral" valor={formatarMoeda(rpaGeral)} percentual={calcPerc(rpaGeral, dadosMetas?.meta_rpa_geral)} labelMeta="Meta RPA:" valorMeta={formatarMoeda(dadosMetas?.meta_rpa_geral)} onClickExpandir={() => abrirModalValExp('RPA Geral', formatarMoeda(rpaGeral), 'Receita Por Ativo Geral.')} />
          <CardMetaNova titulo="Ticket Médio" valor={formatarMoeda(tktGeral)} percentual={calcPerc(tktGeral, dadosMetas?.meta_tkt_medio_geral)} labelMeta="Meta Tkt Médio:" valorMeta={formatarMoeda(dadosMetas?.meta_tkt_medio_geral)} onClickExpandir={() => abrirModalValExp('Ticket Médio', formatarMoeda(tktGeral), 'Ticket Médio Geral.')} />
          <CardMetaNova titulo="UPA Geral" valor={upaGeral.toFixed(1)} percentual={calcPerc(upaGeral, dadosMetas?.meta_upa_geral)} labelMeta="Meta UPA:" valorMeta={Number(dadosMetas?.meta_upa_geral||0).toFixed(1)} onClickExpandir={() => abrirModalValExp('UPA Geral', Number(upaGeral || 0).toFixed(2), 'Unidades Por Atendimento (Itens por pedido).')} />
        </div>
        )}
